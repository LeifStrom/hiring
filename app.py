...
def authenticate_google_sheets():
    ...
    try:
        ...
        st.error(f"❌ Credentials file '{CREDENTIALS_FILE}' not found. Please ensure it's in the root directory.")
        st.stop()
    except Exception as e:
        st.error(f"❌ Authentication failed: {str(e)}")
        st.stop()

def get_spreadsheet_by_name(client, sheet_name: str, owner_email: str):
    """Find and open the spreadsheet by name and owner."""
    try:
...
def main():
    ...
    with st.spinner("🔄 Connecting to Google Sheets..."):
        ...
        spreadsheet = get_spreadsheet_by_name(client, SHEET_NAME, "leif@treadathletics.com")
